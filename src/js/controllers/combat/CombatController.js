...
function attackTurn(attacker, defender, attack, isPlayer, combatLog, combatMessage = []) {
	if (attacker.isAlive() && defender.isAlive() && variables().combat) { // Check if both are alive and combat is still going
		if (typeof attack.runaway !== 'undefined') { // Attacker is attempting to run
			...
			if (hit) { // See if there is a hit
				...
				if (!defender.isAlive()) // Check defender health
					endCombat(!isPlayer)
	...
	} else if (defender.isAlive() && !variables().combat) { // Check to see if defender ran 
